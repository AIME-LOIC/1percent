...
### Fixed
- **Admin OAuth tokens silently issued student-only**: `_issueTokens` parsed the stored scope with the client-facing normalizer, which strips the server-granted `admin` scope — so every token, including those of admins who approved consent, came out student-only and Claude lost all platform management tools. Stored scopes are now parsed with a dedicated `parseStoredScopes` (keeps `admin`; the normalizer is only for client-supplied scope). Additionally, the MCP RPC route now resolves admin capability from the **live `profiles.role` on every request** and backfills the scope — so connections made while the bug was live regain the admin toolset on their next tool call, without reconnecting.
