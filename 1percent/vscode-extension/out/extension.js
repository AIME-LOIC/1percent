...
async function activate(context) {
    ...
    context.subscriptions.push(vscode.window.onDidChangeActiveTextEditor(editor => {
        if (editor)
            ...
            codeAnalyzer.clearDecorations(vscode.window.activeTextEditor);
    ...
    context.subscriptions.push(vscode.commands.registerCommand('1percent.setChallenge', async () => {
        const challenges = await api.getChallenges();
        const selected = await vscode.window.showQuickPick(challenges.map((c) => ({ label: c.title, description: `${c.difficulty} • ${c.challenge_type}`, challenge: c })), { placeHolder: 'Set active challenge for code matching' });
        if (!selected)
            return;
    ...
    context.subscriptions.push(vscode.commands.registerCommand('1percent.submit', async () => {
        ...
        const selected = await vscode.window.showQuickPick(challenges.map((c) => ({
            label: c.title,
        ...
        if (result.passed) {
            ...
            if (result.output)
                out.appendLine(`Output:   ${result.output}`);
            if (result.expected)
                out.appendLine(`Expected: ${result.expected}`);
            if (result.error)
                out.appendLine(`Error:    ${result.error}`);
            out.show();
            vscode.window.showWarningMessage('❌ Solution did not pass. See output for details.');
        }
    }));
    // ---- Run in Terminal ----
    context.subscriptions.push(vscode.commands.registerCommand('1percent.runInTerminal', async () => {
        const editor = vscode.window.activeTextEditor;
        if (!editor) {
        ...
        if (api.isAuthenticated() && output) {
            const action = await vscode.window.showInformationMessage('Run complete. Submit this solution to a challenge?', 'Submit', 'Dismiss');
            if (action === 'Submit') {
    ...
    // ---- Open Terminal ----
    context.subscriptions.push(vscode.commands.registerCommand('1percent.openTerminal', () => {
    ...
    context.subscriptions.push(vscode.commands.registerCommand('1percent.sync', async () => {
        ...
        try {
            await vscode.window.withProgress({
                ...
                await api.syncFile(editor.document.fileName.split('/').pop() || 'untitled', editor.document.getText(), editor.document.languageId);
            });
    ...
    // ---- Open Course ----
    context.subscriptions.push(vscode.commands.registerCommand('1percent.openCourse', (course, module) => {
    ...
    context.subscriptions.push(vscode.commands.registerCommand('1percent.openChallenge', async (challenge) => {
        ...
        vscode.window.showInformationMessage(`🎯 Active: "${challenge.title}" — code is being tracked. Press Ctrl+Shift+Enter to submit.`);
    }));
}
function deactivate() {
    fileTracker?.dispose();
...
function langExt(challengeType) {
    const map = {
...
function defaultStarter(type, title) {
    const starters = {
