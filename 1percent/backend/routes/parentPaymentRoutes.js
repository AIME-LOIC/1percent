...
// NOTE: /create and /:token/pay are hard-blocked below — no payment gateway is
// integrated yet. Remove the `paymentsDisabled` middlewares once a gateway is wired in.

const router = Router();

// Authenticated routes (student)
router.post('/create', authenticate, paymentsDisabled, (req, res, next) => parentPaymentController.createRequest(req, res, next));
...
router.post('/:token/pay', authRateLimit, paymentsDisabled, (req, res, next) => parentPaymentController.processPayment(req, res, next));
